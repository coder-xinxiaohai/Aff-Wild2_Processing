...
def frames_to_label_cat(expression, frames):
    ...
    drop_ids = [i for i in range(len(expression)) if expression[i] == -1 or expression[i] == 7]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--annotations_dir', type=str, default="./Label/Third ABAW Annotations")
    parser.add_argument('--videos_dir', type=str, default="./Image/cropped_aligned")

    args = parser.parse_args()
    annotations_dir = args.annotations_dir
    videos_dir = args.videos_dir

    train_cat = os.path.join(annotations_dir, 'EXPR_Classification_Challenge/Train_Set')
    val_cat = os.path.join(annotations_dir, 'EXPR_Classification_Challenge/Validation_Set')
    train_cont = os.path.join(annotations_dir, 'VA_Estimation_Challenge/Train_Set')
    val_cont = os.path.join(annotations_dir, 'VA_Estimation_Challenge/Validation_Set')
    ...
    for i, filename in enumerate(train_cat_names):
        ...
        for j in range(len(frames_ids)):
            sample = Affwild2_annotation(frame_path=frames_paths[j], expression=expression_array[j], valence=None,
                                         arousal=None)
    ...
    for i, filename in enumerate(train_cont_names):
        frames_paths = sorted(glob.glob(os.path.join(videos_dir, filename, '*.jpg')))
        valence_array, arousal_array, frames_paths, frames_ids = frames_to_label_cont(train_cont_values[i],
                                                                                      frames_paths)
        frames = []
        if len(frames_paths) == 0:
            continue
        for j in range(len(frames_ids)):
            sample = Affwild2_annotation(frame_path=frames_paths[j], valence=valence_array[j], arousal=arousal_array[j],
                                         expression=None)
            frames.append(sample)
                ...
                for frame_cat in data_train_cat[vid1]:
                    for frame_cont in data_cont[vid2]:
                        if frame_cat.frame_path == frame_cont.frame_path:
                            train_mtl.append(
                                Affwild2_annotation(frame_path=frame_cat.frame_path, expression=frame_cat.expression,
    ...
    with open('./result/data_affwild2.pkl', "wb") as w:
        pickle.dump(data, w)
